...
const usersRef = dbRef.child("traducteurs");
...
usersRef.orderByChild("email").equalTo(email).once("value", function(snapshot) {
  ...
  tableToTakeUserrspon[0].forEach(function(userSnapshot){
    ...
    if (result.isConfirmed) {
      // Référence à la base de données des traducteurs
      var usersRef = firebase.database().ref("traducteurs");
      //const UserConnectuser = localStorage.getItem("unserconnect");
      ...
      usersRef.update(updates)
        .then(function() {
          ...
          footer: '<a href="../logintrad.html" id="IDTRANSLATEFORGET11">' + "Connectez-vous" + '</a>',
